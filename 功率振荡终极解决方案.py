...
@计算励磁提供的转矩作用
(1) 输入变量，变量标幺化。 P/SN  Q/SN  I/IN   U/UN
(2) 计算功角和功率因数角。 δ=arctan(P/(Q+U*U/Xq))   θ=arctan(Q/P)  发生振荡时，该公式是否成立，或者是有条件的成立
只有其中的全部为相量时，该公式才成立。但振荡发生时，该公式中的量并不全为相量，而是相量之上还有 
有可能是UI不变，角度变化引起功率振荡。那么

在此均为相量实际值，由于默认PMU计算准确，则相量实际值应该是准确的，因此可以实时测算功角和功率因数角
(3) 根据功角和功率因数角计算直轴电流,交轴电压. Id=I*sin(δ+θ)  Uq=U*cos(δ)
(4) 根据功角和功率因数角计算暂态电势. Eqp = Uq+Id*Xdp 
(5) FFT滤波求 ΔEqp   Δw  ΔPe    
求出来的量仍然是一个实时变化的变化量，进一步需要对这个变化量相量化。
需要注意的是对Δw采用(f-50)/50进行滤波，因为w是一个长期的直流量,直接进行直流滤波即可得到Δw
(6) ∑ΔEqp*Δw     （<0 励磁负阻尼）
对求和进行直流滤波或者积分,有两种手段来得到直流分量.
