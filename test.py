...
(2) δ=arctan(P/(Q+U*U/Xq))   θ=arctan(Q/P)
(3) Id=I*sin(δ+θ)
(4) Eqp = U*cos(δ)+Id*Xdp
(5) FFT滤波求 ΔEqp   Δw  ΔPe    需要注意的是对Δw采用(f-50)/50进行滤波
(6) ∑ΔEqp*Δw     （<0 励磁负阻尼）
...
ax1.plot(temp_t,iyy_ΔEqp,"r-.")
ax1.legend(["ΔEqp "],loc=4)
ax1.set_xlabel("t (s)")
ax1.set_ylabel("ΔEqp")

ax2 = ax1.twinx()
ax2.plot(temp_t,iyy_Δw,'b')
ax2.legend(["Δω"],loc=1)
ax2.set_xlabel("t (s)")
